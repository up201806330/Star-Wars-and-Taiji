class Defbarrel extends CGFobject {
    constructor(scene, base, middle, height, uDivisions, vDivisions, angle) {
        super(scene);
        this.base = base;
        this.middle = middle;
        this.height = height;
        ...
        if (angle != null)  this.angle = angle * 180 / Math.PI;
        else{
    ...
    createSurface() {
        ...
        this.nurbsSurface = new CGFnurbsSurface(
           ...
           [	   	
               ...
               [ // V = 0..3
                    [ r, 0, 0, 1],
                    [ r + H, 0.0,         H / Math.tan(this.angle), 1],
                    [ r + H, 0.0,         L - (H / Math.tan(this.angle)), 1],
                    [ r, 0, L, 1]    
              ],
               // U = 1
               [ // V = 0..3
                    [ r, h, 0, 1],
                    [ r + H, 4/3*(H + r), H / Math.tan(this.angle), 1],
                    [ r + H, 4/3*(H + r), L - (H / Math.tan(this.angle)), 1],
                    [ r, h, L, 1]
               ],
               // U = 2
               [ // V = 0..3
